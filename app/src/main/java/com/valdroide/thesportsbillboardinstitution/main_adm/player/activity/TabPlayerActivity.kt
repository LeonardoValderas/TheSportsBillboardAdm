...
open class TabPlayerActivity : AppCompatActivity(), Communicator {
...
//     ADEFUL
    private fun setPagerAdapter() {
        ...
        val fragments = arrayOf(PlayerCreateFragment(), PlayerUpdateFragment())
        adapter = SectionsPagerAdapter(supportFragmentManager, titles, fragments)
    }
    ...
    override fun refreshAdapter() {
        getPlayerUpdateFragment().refreshAdapter()
    }

    fun getPlayerUpdateFragment(): PlayerUpdateFragment {
        val manager = supportFragmentManager
        return manager
                .findFragmentByTag("android:switcher:" + viewPager.id
                        + ":" + 1) as PlayerUpdateFragment
    }
